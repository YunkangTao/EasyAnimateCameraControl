...
from EasyCamera.model import EasyCamera, pre_process_first_frames, get_inpaint_latents_from_depth
...
class EasyCameraPipeline(DiffusionPipeline):
    r"""
    ...
    def enable_sequential_cpu_offload(self, *args, **kwargs):
        ...
        if hasattr(self.easycamera.easyanimate, "clip_projection") and self.easycamera.easyanimate.clip_projection is not None:
            import accelerate

            accelerate.hooks.remove_hook_from_module(self.easycamera.easyanimate.clip_projection, recurse=True)
            self.easycamera.easyanimate.clip_projection = self.easycamera.easyanimate.clip_projection.to("cuda")
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            if self.easycamera.easyanimate.config.enable_text_attention_mask:
    ...
    def prepare_mask_latents(self, mask, masked_image, batch_size, height, width, dtype, device, generator, do_classifier_free_guidance, noise_aug_strength):
        ...
        if masked_image is not None:
            ...
            if self.easycamera.easyanimate.config.add_noise_in_inpaint_model:
    ...
    def __call__(
        ...
        if self.text_encoder is not None:
            ...
            dtype = self.easycamera.easyanimate.dtype
        ...
        num_channels_transformer = self.easycamera.easyanimate.config.in_channels
        ...
        # if clip_image is not None and self.easycamera.easyanimate.enable_clip_in_inpaint:
        ...
        #     clip_attention_mask = torch.ones([batch_size, self.easycamera.easyanimate.n_query]).to(device, dtype=dtype)
        #     clip_attention_mask_neg = torch.zeros([batch_size, self.easycamera.easyanimate.n_query]).to(device, dtype=dtype)
        ...
        # elif clip_image is None and num_channels_transformer != num_channels_latents and self.easycamera.easyanimate.enable_clip_in_inpaint:
        #     clip_encoder_hidden_states = torch.zeros(
        #         [batch_size, int(self.clip_image_encoder.config.image_size / self.clip_image_encoder.config.patch_size) ** 2, int(self.clip_image_encoder.config.hidden_size)]
        #     ).to(device, dtype=dtype)

        #     clip_attention_mask = torch.zeros([batch_size, self.easycamera.easyanimate.n_query])
        ...
        inpaint_latents = torch.cat([inpaint_latents] * 2, dim=0) if self.do_classifier_free_guidance else inpaint_latents

        # if mask_video is not None:
        #     if (mask_video == 255).all():
        #         # Use zero latents if we want to t2v.
        #         if self.easycamera.easyanimate.resize_inpaint_mask_directly:
        ...
        #     if num_channels_latents + num_channels_mask + num_channels_masked_image != self.easycamera.easyanimate.config.in_channels:
        #         raise ValueError(
        #             f"Incorrect configuration settings! The config of `pipeline.transformer`: {self.easycamera.easyanimate.config} expects"
        #             f" {self.easycamera.easyanimate.config.in_channels} but received `num_channels_latents`: {num_channels_latents} +"
        ...
        grid_height = height // 8 // self.easycamera.easyanimate.config.patch_size
        grid_width = width // 8 // self.easycamera.easyanimate.config.patch_size
        if self.easycamera.easyanimate.config.get("time_position_encoding_type", "2d_rope") == "3d_rope":
            base_size_width = 720 // 8 // self.easycamera.easyanimate.config.patch_size
            base_size_height = 480 // 8 // self.easycamera.easyanimate.config.patch_size

            grid_crops_coords = get_resize_crop_region_for_grid((grid_height, grid_width), base_size_width, base_size_height)
            image_rotary_emb = get_3d_rotary_pos_embed(
                self.easycamera.easyanimate.config.attention_head_dim,
                grid_crops_coords,
            ...
            base_size = 512 // 8 // self.easycamera.easyanimate.config.patch_size
            grid_crops_coords = get_resize_crop_region_for_grid((grid_height, grid_width), base_size, base_size)
            image_rotary_emb = get_2d_rotary_pos_embed(self.easycamera.easyanimate.config.attention_head_dim, grid_crops_coords, (grid_height, grid_width))
        ...
        self.easycamera.easyanimate.to(device, dtype)
        inpaint_latents = inpaint_latents.to(device, dtype)
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.easycamera.easyanimate(
                    latent_model_input,
                    t_expand,
