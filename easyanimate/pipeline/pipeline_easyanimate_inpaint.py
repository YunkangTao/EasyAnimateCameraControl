...
from diffusers.utils import BACKENDS_MAPPING, BaseOutput, deprecate, is_bs4_available, is_ftfy_available, is_torch_xla_available, logging, replace_example_docstring
from diffusers.utils.torch_utils import randn_tensor
from einops import rearrange
from PIL import Image
from tqdm import tqdm
from transformers import BertModel, BertTokenizer, CLIPImageProcessor, Qwen2Tokenizer, Qwen2VLForConditionalGeneration, CLIPVisionModelWithProjection, T5EncoderModel, T5Tokenizer
...
def add_noise_to_reference_video(image, ratio=None, generator=None):
    ...
    if generator is not None:
        image_noise = torch.randn(image.size(), generator=generator, dtype=image.dtype, device=image.device) * sigma[:, None, None, None, None]
    else:
...
class EasyAnimateInpaintPipeline(DiffusionPipeline):
    ...
    def encode_prompt(
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            if type(tokenizer) in [BertTokenizer, T5Tokenizer]:
                ...
                if self.transformer.config.enable_text_attention_mask:
                    # Inference: Generation of the output
                    negative_prompt_embeds = text_encoder(input_ids=text_input_ids, attention_mask=negative_prompt_attention_mask, output_hidden_states=True).hidden_states[-2]
                else:
                    raise ValueError("LLM needs attention_mask")
                negative_prompt_attention_mask = negative_prompt_attention_mask.repeat(num_images_per_prompt, 1)
    ...
    def __call__(
        ...
        if mask_video is not None:
            if self.transformer.config.get("enable_zero_in_inpaint", True) and (mask_video == 255).all():
                ...
                if num_channels_transformer != num_channels_latents:
                    ...
                    # print("max value of masked_video", masked_video.max())
                    # print("min value of masked_video", masked_video.min())
        ...
        video_masked_video = self.decode_latents(masked_video_latents)

        # Convert to tensor
        if output_type == "latent":
            video = torch.from_numpy(video)
            video_masked_video = torch.from_numpy(video_masked_video)

        # Offload all models
        self.maybe_free_model_hooks()

        if not return_dict:
            return video, video_masked_video
