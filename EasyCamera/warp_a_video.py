...
from extern.DepthAnythingV2.metric_depth.depth_anything_v2.dpt import DepthAnythingV2
...
def prepare_frames(video_file, video_length):
    # 创建视频捕捉对象
...
def prepare_camera_poses(camera_pose_file, video_length):
    whole_camera_para = []
    ...
    with open(camera_pose_file, 'r', encoding='utf-8') as file:
        ...
        # 确保文件至少有两行（标题行 + 至少一行数据）
        if len(lines) < 2:
            print("文件内容不足两行，无法读取数据。")
            return whole_camera_para

        # 跳过第一行，从第二行开始，只处理前 video_length 行
        for idx, line in enumerate(lines[1 : 1 + video_length], start=2):
            # 去除首尾空白字符并按空格分割
...
def prepare_models(dav2_outdoor, dav2_model, depth_anything_path):
    ...
    # depth_anything.load_state_dict(torch.load(depth_anything_path, map_location='cpu'))
